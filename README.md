...
通过 WebSocket 实时采集好友上下线、世界切换、Avatar/状态变化并入库。以 57 个 MCP 工具向 AI Agent 暴露全部能力——不只查询，还涵盖社交互动（戳戳/邀请/好友请求）、媒体管理（emoji/相册/图库）、群组操作、推荐系统等。附带 Hermes 插件实现进程托管（自动拉起 + 崩溃自愈）。
...
按能力域划分的功能概览，全部 57 个 MCP 工具与详细参数见下方「🔌 MCP 工具」。

### 📡 实时监控与认证自愈

- WebSocket 实时采集好友上下线、世界切换、Avatar/状态变化，即时入库
- 断线自动重连（指数退避）+ cookie 过期自动 OTP 邮箱取码登录，全链路无人值守

### 🤖 智能推荐

- AI 好友推荐：熟悉度 + 收藏夹权重 + 房间场景 + 实例人数/类型综合评分，推荐当前最值得加入的房间
- 偏好学习：自然语言设置偏好（如「不喜欢人太多」→ 爆满重罚），记录实际选择后自动学习并调整权重

### 💬 社交互动

- 戳戳（Boop）、邀请进房、请求加入、好友请求/删除，内置限流防封
- 开房引擎：`open_world` / `create_instance` / `invite_myself`，命名管道直发 1 步进房，失败静默回退 API 邀请

### 🖼 媒体管理

- VRC+ 相册（Prints）/ 图库（Gallery）/ 自定义 boop 表情：上传、下载、删除、列表
...
- 事件历史、同屏交叉查询、上线规律分析、一周游戏周报
- 昵称映射、世界名缓存（懒刷新防改名陈旧）、世界备注与变更历史、新世界追踪、关注名单
- 从 VRCX 一键迁移历史数据（上下线/位置/Avatar/状态/Bio、好友列表、世界缓存）

### 🛡 运维

- 数据库自动备份：启动 + 每 24h WAL 在线备份（无需停机），保留最近 2 份，可手动触发
...
| Skill | 内容 | 适用场景 |
|-------|------|----------|
| `skills/vrc-monitor-agent/` | 57 个 MCP 工具清单、5 大查询工作流（在线/同屏/时间线/上线规律/昵称）、常见陷阱、健康检查 | 日常好友查询与社交操作 |
...
│   ├── backup.js               # 数据库在线备份
│   ├── mcp-definitions.js      # MCP 工具定义（57 个工具）
